...
laurasLarderControllers.controller('introLLCtrl', ['$scope', '$modal', '$location', 'LL_OpenSave', '$translate',
    function($scope, $modal, $location, LL_OpenSave, $translate) {
        $scope.steps = ['LL intro-1', 'LL intro-2', 'LL intro-3', 'LL intro-4', 'LL intro-5', 'LL intro-6', 'LL intro-7', 'LL intro-8', 'LL intro-9', 'LL intro-10', 'LL intro-11', 'LL intro-12', 'LL intro-13', 'LL intro-14'];
        $scope.step = 0;
        ...
        $scope.openDiet = function() {
            ...
            LL_OpenSave.openDiet($scope);
        };
        $scope.chooseLanguaje = function(key) {
            $translate.use(key);
            $scope.languaje = key;
            $scope.handleNextIntroduction();
        };
...
/**  Don´t isunderstand the controller below, i have used the Introduction controller, copied nad pasted. But it is really the extro  **/
laurasLarderControllers.controller('extroLLCtrl', ['$scope', '$location', '$translate',
    function($scope, $location, $translate) {
        $scope.steps = ['LL extro-1', 'LL extro-2', 'LL extro-3', 'LL extro-about'];
        $scope.step = 0;
        $scope.languaje = $translate.use();
        ...
        $scope.home = function() {
            $location.path('/LL-introduction');
        };
        //testing
        //$scope.languaje = 'cym';        
    }]);

laurasLarderControllers.controller('inputDataLLCtrl', ['$scope', 'ProductsRepository', '$modal', 'LL_items', 'RNIsRepository', '$location', 'LL_OpenSave', '$translate',
    function($scope, ProductsRepository, $modal, LL_items, RNIsRepository, $location, LL_OpenSave, $translate) {
        /** Some objects in the scope  **/
        $scope.dietToSave = {};
        $scope.languaje = $translate.use();
        ...
        /** Some other objects in the scope that need to be initialitated  **/
        $scope.steps = ['user_info', 'add_product_to_meal', 'show_subcategories_and_products']; //I ODN THINK THIS VAR IS IN USE
        ...
        $scope.userInfo = {//information about the user, default value
            gender: 'female',
            age_group: '19-50'
        };
        var productIndex = 0; //index for assigning an id to each product we add to the weeklyMeals table
        $scope.storage = localStorage;
        ...
        //$scope.NumberOfImagesLoading = 0; // we use it in the "imageonload" directive to display/hide the loading spinner
        //Load larders
        ProductsRepository.populateLarders($scope.larders, $scope.list_of_category_graphics, $scope.products_list);


        /** Load Diet - If we have clicked on "Open Diet" in the Introduction we need to load the relevant objects and siplay the weekly_table **/
        if (LL_items.getLoadingDiet() === true) {
            ...
            $translate.use(LL_items.getLanguaje());
            $scope.filling_up_first_day = false;
            $scope.helper_step = "loading_diet";
        }
        /** End Load Diet  **/
...
//templateUrl: 'partials/modalIntroduceUserInfo.html',
        ...
        /*-------------------------------------------------*/
        /***  Controller ModalWelcomePageCtrl   NOT IN USE       
         /*-------------------------------------------------*/
...
laurasLarderControllers.controller('resultsLLCtrl', ['$scope', 'RNIsRepository', 'LL_items', '$filter', '$modal', 'LL_OpenSave', '$location', '$translate',
    function($scope, RNIsRepository, LL_items, $filter, $modal, LL_OpenSave, $location, $translate) {
        ...
        $scope.setGHADR_step = function(macronutrient) {
            $scope.GHADR_step = macronutrient;
        };
        $scope.setVitMin_step = function(vitmin) {
            $scope.vitmin_info_step = vitmin;
        }
                ...
                case 'general_health_and_disease_risk_1':
                    switch (args.reason) {
                        case 'macronutrient_clicked':
                            ...
                            openMacronutrientsModal(args.macronutrient);
                            break;
                        case 'next':
                            //$scope.step = 'VitMin';
        ...
        $scope.nutrientInRecommendedLevels = function(nutrient, nutrient_group) {
            ...
            if (nutrient === 'ENERGY') {
                if (user_intake_mass <= 1.1 * recommended_intake) //The 'ENERGY' intake is in recommended level when it is in the 10% of the RNI
                    return true;
                else
                    return false;
            }
            if (nutrient === 'SALT') {
                if (user_intake_mass <= 6000) //We should be eating no more than 2400mg of sodium = 6000mg of salt
                    return true;
                else
                    return false;
            }
        ...
        var openMacronutrientsModal = function(macronutrient) {
            $scope.modalMacronutrient = $modal.open({
                templateUrl: 'partials/modalMacronutrients.html',
                windowClass: 'modalMacronutrients',
                scope: $scope
            });
        };
        var openVitMinModal = function(vitmin) {
            console.log(vitmin);
            $scope.modalMacronutrient = $modal.open({
                templateUrl: 'partials/modalVitMin.html',
                windowClass: 'modalVitMin',
                scope: $scope
            });
        };
        ...
        var updateUserDiet = function() {
            ...
            angular.forEach(['breakfast', 'lunch', 'dinner', 'snacks_and_drinks'], function(meal, index) {
                ...
                angular.forEach(products_in_meal, function(product, product_index) {
                    ...
                    nutritional_data_total_this_meal.Macronutrients.SALT = 2.5 * nutritional_data_total_this_meal.Macronutrients.NA;//special instruction from Laura, this is how you calculate the SALT
                });
                ...
                // Add everything to the userDietObject (except Top Five Emitters)
            ...
            /**   Top Five Emitters  **/
            //Sort list of products accordding to their emissions
            ...
            $scope.userDiet.topFiveEmitters = [];
            var topFiveEmittersIndex = 0;
            var listOfProductsIndex = 0;
            var topEmitterTimes = 1;
            console.log(listOfProducts);
            while (topFiveEmittersIndex < 5) {
                while (listOfProductsIndex < listOfProducts.length && listOfProducts[listOfProductsIndex].food_group === 'Drinks') // we skipp the Drinks from the top 5 emitters
                    listOfProductsIndex++;
                if (listOfProductsIndex < listOfProducts.length) {
                    $scope.userDiet.topFiveEmitters[topFiveEmittersIndex] = angular.copy(listOfProducts[listOfProductsIndex]);
                    topEmitterTimes = 1;
                    listOfProductsIndex++;
                    while (listOfProductsIndex < listOfProducts.length && $scope.userDiet.topFiveEmitters[topFiveEmittersIndex].key === listOfProducts[listOfProductsIndex].key) {
                        listOfProductsIndex++;
                        topEmitterTimes++;
                    }
                    $scope.userDiet.topFiveEmitters[topFiveEmittersIndex].numberOfTimes = topEmitterTimes;
                }
                topFiveEmittersIndex++;
            }
        ...
        var iniBarGraph = function() {
            ...
            angular.forEach($scope.userDiet.ghgFoodGroupsPerDay, function(emissions, food_group) {
                ...
                switch (food_group) {
                    ...
                    case 'Starchy_foods':
                        bar_color = '#f8d835';
                        break;
                    case 'Foods_high_in_fat,_sugar,_salt':
                        bar_color = '#8e576c';
                        break;
                    case 'Eggs':
                        bar_color = '#e1e0ad';
                    ...
                    case 'Dairy':
                        bar_color = '#6eb1de';
                        break;
                    case 'High_protein_meat_alternatives':
                        bar_color = '#ec8fec';
                        break;
                    case 'Vegetables':
                        bar_color = '#669579';
                        break;
                    case 'Condiments,_soups_and_sauces':
                        bar_color = '#d6b99f';
                        break;
                ...
                data.data_values.push({
                    ...
                    color: bar_color,
                    bars: {show: true, barWidth: 0.6, fillColor: bar_color, order: index + 1, align: "center"}
                });
            ...
            data.data_values = [
                {data: [[0, 8]], color: '#be627b', bars: {show: true, barWidth: 0.5, fillColor: '#be627b', order: index + 1, align: "center"}
                },
                {data: [[1, $scope.userDiet.ghgTotalPerDay]], color: '#faa61a', bars: {show: true, barWidth: 0.5, fillColor: '#faa61a', order: index + 1, align: "center"}
                },
                {data: [[2, 4]], color: '#32976e', bars: {show: true, barWidth: 0.5, fillColor: '#32976e', order: index + 1, align: "center"}
                }
            ];
        ...
        $scope.step = 'main_results';
        //$scope.step = 'top_five_emitters'; //testing
